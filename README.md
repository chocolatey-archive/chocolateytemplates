...
```cmd
 ...
 warmup addTextReplacement __CHOCO_PKG_MAINTAINER_REPO__ "Your Repository"
 warmup addTextReplacement __CHOCO_AUTO_PKG_MAINTAINER_REPO__ "Your Choco Automatic Packages Repository (could be same as other)"
 ...
 * The package maintainer (old term: owner) name (__CHOCO_PKG_MAINTAINER_NAME__) would be you.
 * Your packages repository (__CHOCO_PKG_MAINTAINER_REPO__) is part of a github repo just **ferventcoder/nugetpackages** if your repository is https://github.com/ferventcoder/nugetpackages. This is only used for image urls.
 * Your chocolatey automatic packages repository (__CHOCO_AUTO_PKG_MAINTAINER_REPO__) could be the same as your regular packages repository. This is also the same as package maintainer repo. This is only used for image urls.
